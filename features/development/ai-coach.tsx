    ...
    <Card className="rounded-2xl ring-primary/20 [--card-spacing:--spacing(6)]">
      <CardContent className="space-y-5">
        ...
        <div ref={resultRef} tabIndex={-1} className="outline-none" aria-live="polite" aria-atomic="true">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">{loading ? "AI готовит разбор вашей цели, объяснение выбора и практический первый шаг. Ответ появится прямо в этом блоке." : isAI ? "Разбор вашего маршрута от AI" : error ? error.detail : "Получите персональный разбор: что развивать в первую очередь, почему эти активности подходят вам и с чего начать."}</p>
          {feedback && <div className="mt-4 space-y-3 rounded-xl border border-primary/20 bg-primary/5 p-4"><p className="whitespace-pre-line break-words text-sm leading-7">{feedback.summary}</p><p className="text-xs text-muted-foreground">Комментарий AI на основе профиля. Расчётные показатели показаны отдельно в карточках.</p></div>}
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">{loading ? "Готовим объяснение и первый шаг" : isAI ? "Персональный разбор · OpenAI" : "Запрос только по вашему нажатию"}</span>
        </div>
        ...
        return <Card key={event.event_id} className={`rounded-2xl [--card-spacing:--spacing(5)] ${isAI && index === 0 ? "ring-primary/40" : ""}`}>
          <CardContent>
            ...
            {commentary && <div className="mt-5 space-y-4">
              <div><h5 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-primary"><Sparkles className="size-3.5" />Почему AI рекомендует это вам</h5><p className="whitespace-pre-line break-words text-sm leading-7">{commentary.explanation}</p></div>
              <div className="rounded-xl border border-primary/15 bg-primary/5 p-4"><h5 className="mb-2 text-xs font-semibold">С чего начать · совет AI</h5><p className="whitespace-pre-line break-words text-sm leading-relaxed">{commentary.firstStep}</p></div>
            </div>}
            <div className="mt-4 space-y-2 rounded-xl bg-muted/50 p-3.5">
              <p className="mb-3 text-xs text-muted-foreground">Ожидаемый прирост · расчёт ядра</p>
